    ...
    <div className="bg-card border-b md:border-b-0 md:border-r border-border p-6 flex flex-col gap-6 h-full transition-colors duration-300 overflow-y-auto">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {INTERVAL_GRID.map((interval) => {
            const isActive = currentIntervals.includes(interval.semitones);
            return (
              <div
                key={interval.label}
                title={interval.fullLabel}
                className={`
                  flex flex-col items-center justify-center py-2 rounded border text-xs font-bold transition-all cursor-default select-none
                  ${isActive
